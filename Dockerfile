...
RUN git clone --branch docker-configure-changes https://github.com/Chia-Network/chia-blockchain.git \
&& cd chia-blockchain \
&& chmod +x install.sh \
...
COPY ./old-entrypoint old-entrypoint

ENTRYPOINT ["/bin/bash"]
ENTRYPOINT ["bash", "./old-entrypoint"]
